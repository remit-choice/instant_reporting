                    ...
                    $customers = OnlineCustomer::whereHas('transactions', $customers_transactions)->orwhereDoesntHave('transactions')->with('transactions', $customers_transactions)
                        ...
                        ->get()->groupBy('register_date')->map(function ($row) {
                            ...
                            foreach ($row->groupBY('country') as $customer_countries => $inner_row) {
                                ...
                                //     $no_attempt_count += $inner_sub_rows->no_attempt_count;
                                // }
                                // return json_decode(json_encode([$country_customer_counts]));


                                // $inner_row->map(function ($inner_rows) use ($no_attempt_count, $inner_row, $no_attempt_counts, $inner_transacting_count, $inner_non_transacting_count) {


                                // $inner_rows['transacting_count'];
                                // return $inner_transacting_count = $inner_rows->sum();
                                // $inner_non_transacting_count = $inner_rows->sum();

                                // dd($inner_non_transacting_count);
