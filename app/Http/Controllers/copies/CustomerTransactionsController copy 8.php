...
class CustomerTransactionsController extends Controller
{
        ...
        if (FacadesRequest::isMethod('post')) {
            ...
            if (!empty($request->customer_country) && empty($request->beneficiary_country) && !empty($request->date_from) && empty($request->date_to)) {
                ...
                if ($request->customer_country == 'All Customer Countries') {
                    $customers = OnlineCustomer::where('register_date', $customer_date_from)->with('transactions')
                        ->withCount([
                        ...
                        ->groupBy('country')
                        ->map(function ($row) use ($custom) {
                            return $this->filter_funciton($row, $custom);
                        });
                    ...
                    $customers = OnlineCustomer::where('register_date', $customer_date_from)->with('transactions', $customers_transactions)
                        ->withCount([
                            'transactions as transacting_count' => function ($query) {
                                if ($query->where('status', 'Paid') || $query->where('status', 'Ok') || $query->where('status', 'Compliance Hold')) {
                                    return 1;
                        ...
                        ])->where('country', $customer_country)->get()->groupBy('country')->map(function ($row) use ($custom) {
                            return $this->filter_funciton($row, $custom);
                        });
                    // dd($customers->toArray());
                }
                ...
                if ($request->beneficiary_country == 'All Beneficiary Countries') {
                    $customers = OnlineCustomer::where('register_date', $customer_date_from)->with('transactions')
                        ...
                        ->get()->groupBy('country')->map(function ($row) use ($custom) {
                            return $this->filter_funciton($row, $custom);
                        });
                } else {
                    ...
                    $custom = 0;
                    $customers = OnlineCustomer::where('register_date', $customer_date_from)->whereHas('transactions', $customers_transactions)->orWhereDoesntHave('transactions')
                        ->withCount([
                            'transactions as transacting_count' => function ($query) {
                                if ($query->where('status', 'Paid') || $query->where('status', 'Ok') || $query->where('status', 'Compliance Hold')) {
                                    return 1;
                ...
                if ($request->customer_country == 'All Customer Countries') {
                    $customers = OnlineCustomer::whereBetween('register_date', [$customer_date_from, $customer_date_to])->with('transactions')
                        ->withCount([
                            'transactions as transacting_count' => function ($query) {
                                if ($query->where('status', 'Paid') || $query->where('status', 'Ok') || $query->where('status', 'Compliance Hold')) {
                                    return 1;
                        ...
                        ->groupBy('country')->map(function ($row) use ($custom) {
                            return $this->filter_funciton($row, $custom);
                        });
                } else {
                    ...
                    $get_coustomer_country = Currency::where('name', 'LIKE', '%' . $request->customer_country . '%')->select('iso3')->first();
                    $customer_country = $get_coustomer_country->iso3;
                    $customers = OnlineCustomer::whereBetween('register_date', [$customer_date_from, $customer_date_to])->with('transactions', $customers_transactions)
                        ->withCount([
                            'transactions as transacting_count' => function ($query) {
                                if ($query->where('status', 'Paid') || $query->where('status', 'Ok') || $query->where('status', 'Compliance Hold')) {
                                    return 1;
    ...
    public function filter_funciton($row, $custom)
    {
        $array = [];
        $no_attempt_counts = 0;
        ...
        return json_decode(json_encode(['data' => $array, 'custom' => $custom, 'transacting_count' => $transacting_count, 'non_transacting_count' => $non_transacting_count, 'no_attempt_count' => $no_attempt_counts]));
    }
}
 // $customers = OnlineCustomer::where('register_date', $customer_date_from)->where(function ($query) use ($customers_transactions, $beneficiary_country) {
                    //     $query->whereHas('transactions', $customers_transactions)->orWhereDoesntHave('transactions');
                    // })->with('transactions')->withCount([
                    //     'transactions as transacting_count' => function ($query) {
                    //         if ($query->where('status', 'Paid') || $query->where('status', 'Ok') || $query->where('status', 'Compliance Hold')) {
                    //             return 1;
                    //         }
